...
runThreadNetwork ThreadNetworkArgs
  ...
  where
    ...
    forkNode epochInfo varRNG btime registry pInfo nodeInfo txs0 = do
      ...
      -- We cannot issue the immediate transactions until we've synced enough
      -- blocks that an up-to-date delegation transaction is valid compared to
      -- the (epoch of the) slot of the ledger state.
      --
