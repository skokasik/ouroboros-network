...
showBlockHeaderSize immDB epochInfo rr = do
  ...
  where
    ...
    go maxBlockHeaderSizeRef blk =
        case blk of
          Byron.ByronBlock (Chain.ABOBBlock regularBlk) _ _ ->
            go' maxBlockHeaderSizeRef (blockSlot blk) regularBlk
          _otherwise ->
            return () -- Skip EBBs

    go' :: IORef Natural -> SlotNo -> Chain.ABlock ByteString -> IO ()
    go' maxBlockHeaderSizeRef slotNo block = do
        void $ atomicModifyIORef' maxBlockHeaderSizeRef $ \maxSz ->
          let maxSz' = max maxSz blockHeaderSz
          in (maxSz', maxSz')
        epochSlot <- relativeSlotNo epochInfo slotNo
        putStrLn $ intercalate "\t" [
            show slotNo
          , show epochSlot
          , ("Block header size = " <> show blockHeaderSz)
          ]
      where
        Chain.ABlock { blockHeader } = block
        blockHeaderSz = Chain.headerLength blockHeader
...
showBlockTxsSize immDB epochInfo rr = processAll immDB rr go
  where
    go :: ByronBlock -> IO ()
    go blk =
        case blk of
          Byron.ByronBlock (Chain.ABOBBlock regularBlk) _ _ ->
            go' (blockSlot blk) regularBlk
          _otherwise ->
            return () -- Skip EBBs

    go' :: SlotNo -> Chain.ABlock ByteString -> IO ()
    go' slotNo block = do
        epochSlot <- relativeSlotNo epochInfo slotNo
        putStrLn $ intercalate "\t" [
            show slotNo
      ...
      where
        ...
        txSerializedLength :: Chain.ATxAux ByteString -> Int
        txSerializedLength = BS.length . Chain.aTaAnnotation

        txsSerializedLength :: [Chain.ATxAux ByteString] -> Int
        txsSerializedLength txs = foldl' (+) 0 (map txSerializedLength txs)
