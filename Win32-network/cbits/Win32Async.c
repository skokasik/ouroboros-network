...
// by 'Win32_AsyncRead' or 'Win32_AsyncWrite'
DllExport
...
// be freed by 'Win32_GetQueuedCompletionStatus'.
//
// Aruments:
...
// obtained via Win32_GetQueuedCompletionStatus.
DllExport
int HsAsyncRead(HANDLE handle, void *buf, DWORD size,
                    void *userData)
{
    PER_IO_DATA *perIoDataPtr;
    DWORD numBytesRead;
...
// 'Win32_GetQueuedCompletionStatus'.
//
// arguments:
// HANDLE handle    - handle which must be initialised with FILE_FLAG_OVERLAPPED
...
DllExport
int HsAsyncWrite(HANDLE handle, void *buf, int size,
                 void *userData)
{
    PER_IO_DATA *perIoDataPtr;
