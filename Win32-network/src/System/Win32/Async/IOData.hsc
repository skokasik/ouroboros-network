...
instance Storable (Overlapped Async) where
    sizeOf    (Overlapped ovl) = sizeOf ovl
    alignment (Overlapped ovl) = alignment ovl

    -- TODO: use a safe version of castPtr
    poke buf (Overlapped ovl) = poke (castPtr buf) ovl
    peek buf = Overlapped <$> peek (castPtr buf)


instance Storable (Overlapped WsaAsync) where
    sizeOf    (WsaOverlapped ovl) = sizeOf ovl
    alignment (WsaOverlapped ovl) = alignment ovl

    poke buf (WsaOverlapped ovl) = poke (castPtr buf) ovl
    peek buf = WsaOverlapped <$> peek (castPtr buf)
