...
import System.Win32.CompletionPort  as Win32 (createIoCompletionPort)
...
createIOCompletionPort =
    IOCompletionPort <$>
      Win32.createIoCompletionPort Win32.iNVALID_HANDLE_VALUE nullPtr 0 maxBound
...
waitForCompletion :: (StablePtr (MVar (Either ErrCode Int)) -> IO CInt)
                  -> IO Int
          ...
          then do
            ...
            case res' of
                ...
                Left e | e == eRROR_HANDLE_EOF
                       -> pure 0
                       | otherwise
                       -> Win32.failWith "waitForCompletion" e
          else
            -- An error occurred.
            if errCode == eRROR_HANDLE_EOF
              then return 0
              else Win32.failWith "waitForCompletion" errCode
      else
        -- The operation is completed synchronously.
