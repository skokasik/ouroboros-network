...
connectTo_V1
  ...
  -- ^ remote address
  -> IO ()
connectTo_V1 createChannel sn muxTracer handshakeTracer peeridFn versionData application =
  connectTo
    createChannel sn
    muxTracer
    handshakeTracer
...
withServer_V1
  ...
  -> (Async () -> IO t)
  -> IO t
withServer_V1 muxTracer handshakeTracer tbl createChannel sn addr peeridFn versionData application =
    withServer
      muxTracer
      handshakeTracer
      tbl
